...
export default function HomePage() {
  ...
  const [newChar, setNewChar] = useState({
    name: '',
    world: '', // ← agora é string vazia, não valor fixo
  ...
  const handleAddCharacter = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('characters').insert({
      ...
      world: newChar.world.trim().toLowerCase().replace(/\b\w/g, l => l.toUpperCase()), // ← aceita qualquer mundo
  ...
  <ul className="space-y-3">
    {characters.map((char) => (
      ...
      {showForm && (
            ...
            {/* 👇 CAMPO DE MUNDO COMO INPUT LIVRE */}
            <input
                type="text"
                placeholder="Mundo (ex: Calmera, Antica, Secura...)"
                value={newChar.world}
